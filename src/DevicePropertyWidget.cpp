    ...
    try {
        ...
        if (isReadOnly) {
            ...
            if (!editor && (isInteger || isFloat)) {
                const bool hasLimits = propertyInfo.hasLimits();
                const double lowerLimit = propertyInfo.lowerLimit();
                const double upperLimit = propertyInfo.upperLimit();
                ...
                if (isInteger) {
                    ...
                    if (hasLimits) {
                        spinBox->setRange(static_cast<int>(lowerLimit), static_cast<int>(upperLimit));
                        const int range = static_cast<int>(upperLimit - lowerLimit);
                        if (range > 10000) spinBox->setSingleStep(1000);
                        else if (range > 1000) spinBox->setSingleStep(100);
                        else if (range > 100) spinBox->setSingleStep(10);
                        else spinBox->setSingleStep(1);
                    } else {
                    ...
                    connect(spinBox, &QSpinBox::editingFinished, this,
                            [this, spinBox, deviceLabel, propertyName]() {
                                QString error;
                                const QString newValue = QString::number(spinBox->value());
                                if (!m_scopeonecore->setPropertyValue(deviceLabel, propertyName, newValue, &error)) {
                                    emit errorOccurred(QString("Failed to set property %1.%2: %3")
                                        .arg(deviceLabel, propertyName, error));
                                    return;
                    ...
                    QDoubleSpinBox* doubleSpinBox = new NoWheelDoubleSpinBox();
                    doubleSpinBox->setFocusPolicy(Qt::StrongFocus);
                    doubleSpinBox->setKeyboardTracking(false);
                    if (hasLimits) {
                        doubleSpinBox->setRange(lowerLimit, upperLimit);
                    ...
                    connect(doubleSpinBox, &QDoubleSpinBox::editingFinished, this,
                            [this, doubleSpinBox, deviceLabel, propertyName]() {
                                QString error;
                                const QString newValue = QString::number(doubleSpinBox->value());
                                if (!m_scopeonecore->setPropertyValue(deviceLabel, propertyName, newValue, &error)) {
                                    emit errorOccurred(QString("Failed to set property %1.%2: %3")
                                        .arg(deviceLabel, propertyName, error));
                                    return;

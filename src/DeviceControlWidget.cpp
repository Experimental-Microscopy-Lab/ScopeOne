...
QWidget* DeviceControlWidget::createStageGroup()
{
    ...
    m_xyStepSpinBox = new QDoubleSpinBox();
    m_xyStepSpinBox->setRange(0.001, 100000.0);
    m_xyStepSpinBox->setDecimals(3);
    m_xyStepSpinBox->setValue(10.0);
    xyStepLayout->addWidget(m_xyStepSpinBox, 0, 1);

    xyStepLayout->addWidget(new QLabel(">> (um):"), 1, 0);
    m_xyBigStepSpinBox = new QDoubleSpinBox();
    m_xyBigStepSpinBox->setRange(0.001, 100000.0);
    m_xyBigStepSpinBox->setDecimals(3);
    m_xyBigStepSpinBox->setValue(100.0);
    xyStepLayout->addWidget(m_xyBigStepSpinBox, 1, 1);
    ...
    m_zStepSpinBox = new QDoubleSpinBox();
    m_zStepSpinBox->setRange(0.001, 100000.0);
    m_zStepSpinBox->setDecimals(3);
    m_zStepSpinBox->setValue(1.0);
    zStepLayout->addWidget(m_zStepSpinBox, 0, 1);

    zStepLayout->addWidget(new QLabel(">> (um):"), 1, 0);
    m_zBigStepSpinBox = new QDoubleSpinBox();
    m_zBigStepSpinBox->setRange(0.001, 100000.0);
    m_zBigStepSpinBox->setDecimals(3);
    m_zBigStepSpinBox->setValue(10.0);
    zStepLayout->addWidget(m_zBigStepSpinBox, 1, 1);
    ...
    connect(m_xyUpButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(0.0, m_xyStepSpinBox->value());
    });
    connect(m_xyDownButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(0.0, -m_xyStepSpinBox->value());
    });
    connect(m_xyLeftButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(-m_xyStepSpinBox->value(), 0.0);
    });
    connect(m_xyRightButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(m_xyStepSpinBox->value(), 0.0);
    });
    connect(m_xyBigUpButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(0.0, m_xyBigStepSpinBox->value());
    });
    connect(m_xyBigDownButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(0.0, -m_xyBigStepSpinBox->value());
    });
    connect(m_xyBigLeftButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(-m_xyBigStepSpinBox->value(), 0.0);
    });
    connect(m_xyBigRightButton, &QPushButton::clicked, this, [this]() {
        moveXYStage(m_xyBigStepSpinBox->value(), 0.0);
    });
    connect(m_zUpButton, &QPushButton::clicked, this, [this]() {
        moveZStage(m_zStepSpinBox->value());
    });
    connect(m_zDownButton, &QPushButton::clicked, this, [this]() {
        moveZStage(-m_zStepSpinBox->value());
    });
    connect(m_zBigUpButton, &QPushButton::clicked, this, [this]() {
        moveZStage(m_zBigStepSpinBox->value());
    });
    connect(m_zBigDownButton, &QPushButton::clicked, this, [this]() {
        moveZStage(-m_zBigStepSpinBox->value());
    });
...
void DeviceControlWidget::updateStageControlsEnabled()
{
    ...
    if (m_xyStepSpinBox) m_xyStepSpinBox->setEnabled(hasXY);
    if (m_xyBigStepSpinBox) m_xyBigStepSpinBox->setEnabled(hasXY);
    if (m_zStepSpinBox) m_zStepSpinBox->setEnabled(hasZ);
    if (m_zBigStepSpinBox) m_zBigStepSpinBox->setEnabled(hasZ);
...
void DeviceControlWidget::updateStagePositions()
{
    ...
    if (xyLabel.isEmpty()) {
        ...
        if (m_scopeonecore->readXYPosition(xyLabel, x, y)) {
            m_xPosLabel->setText(QString("X: %1").arg(QString::number(x, 'f', 3)));
            m_yPosLabel->setText(QString("Y: %1").arg(QString::number(y, 'f', 3)));
        } else {
            m_xPosLabel->setText("X: N/A");
            m_yPosLabel->setText("Y: N/A");
        }
    ...
    if (zLabel.isEmpty()) {
        ...
        if (m_scopeonecore->readZPosition(zLabel, z)) {
            m_zPosLabel->setText(QString("Z: %1").arg(QString::number(z, 'f', 3)));
        } else {
            m_zPosLabel->setText("Z: N/A");
        }
    }

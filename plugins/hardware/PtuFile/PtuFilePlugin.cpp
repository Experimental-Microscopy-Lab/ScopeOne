    ...
    namespace
    {
        ...
        bool readHeader(QFile& file, PtuHeader& header, QString& errorMessage)
        {
            ...
            if (recordCount < 0 || recordType < 0
                || !std::isfinite(globalResolution)
                || globalResolution <= 0.0
        ...
        public:
            ...
            {
                const QString filePath = config.sourceSettings
                ...
                settings.detectorChannel = config.sourceSettings
                    .value(QStringLiteral("detectorChannel"), 1).toInt();
                ...
                m_worker = std::jthread([this, settings](std::stop_token stopToken)
                {
        ...
        private:
            ...
            void run(std::stop_token stopToken, const PtuSettings& settings)
            {
                ...
                {
                    const qsizetype count = static_cast<qsizetype>(std::min<quint64>(
                        kRecordsPerChunk, header.recordCount - processedRecords));
                    const QByteArray rawRecords = file.read(count * 4);
                    if (rawRecords.size() != count * 4)
                    {
                    ...
                    if (settings.publishEvents)
                    {
                        ...
                        events.eventTicks.reserve(count);
                        events.eventCodes.reserve(count);
                    }
                    ...
                    processedRecords += static_cast<quint64>(count);
                    if (events.isValid())
                    {

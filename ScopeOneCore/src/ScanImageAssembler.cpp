...
namespace scopeone::core
{
    ...
    {
        m_readyFrames.clear();
        if (!isValid() || chunk.sourceId != m_sourceId)
        {
            return {};
        }
    ...
    QList<ImageFrame> ScanImageAssembler::finish()
    {
        ...
        {
            emitAveragedFrame(m_lastFrameTick);
        }
        m_frameActive = false;
        m_lineActive = false;
        m_lineEventTicks.clear();
    ...
    void ScanImageAssembler::handleMarker(quint64 tick, quint32 code)
    {
        ...
        if (matchesMarker(code, m_config.lineMarker))
        {
            finishLine(tick);
            beginLine(tick);
        }
    ...
    void ScanImageAssembler::finishLine(quint64 tick)
    {
        ...
        if (tick > m_lineStartTick && m_nextRow < m_config.height)
        {
            ...
            const bool reverse = m_config.mirrorHorizontal
                != (m_config.serpentine && (m_nextRow % 2 == 1));
            ...
            for (quint64 eventTick : std::as_const(m_lineEventTicks))
            {
                ...
                const long double phase =
                    static_cast<long double>(eventTick - m_lineStartTick) / duration;
                const long double position = phase <= 0.5L
                    ? phase * 2.0L
                    : (1.0L - phase) * 2.0L;
                const int x = std::min(
                    m_config.width - 1,
                    static_cast<int>(position * m_config.width));
    ...
    void ScanImageAssembler::finishFrame(quint64 tick)
    {
        ...
        if (m_config.averageFrames == 1)
        {
            emitFrame(m_framePixels, tick);
        }
        else
        {
            if (m_accumulatedPixels.isEmpty())
            {
                m_accumulatedPixels.resize(m_framePixels.size());
            }
            for (int index = 0; index < m_framePixels.size(); ++index)
            {
                m_accumulatedPixels[index] += m_framePixels[index];
            }
            ++m_accumulatedFrameCount;
            if (m_accumulatedFrameCount >= m_config.averageFrames)
            {
                emitAveragedFrame(tick);
            }
        }
        m_frameActive = false;
        m_lineActive = false;
    ...
    void ScanImageAssembler::emitFrame(const QVector<quint16>& pixels,
                                        quint64 tick)
    {
    ...
    void ScanImageAssembler::emitAveragedFrame(quint64 tick)
    {
        QVector<quint16> pixels;
        pixels.resize(m_framePixels.size());
        for (int index = 0; index < pixels.size(); ++index)
        {
            pixels[index] = static_cast<quint16>(
                m_accumulatedPixels[index]
                / static_cast<quint64>(m_accumulatedFrameCount));
        }
        emitFrame(pixels, tick);
        m_accumulatedPixels.fill(0);
        m_accumulatedFrameCount = 0;
    }
